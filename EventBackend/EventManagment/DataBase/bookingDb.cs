...
namespace EventManagementproj.DataBase
{
    public class BookingDB
    {
        string connectionString =
            "Data Source=Nuthan\\SQLEXPRESS;Initial Catalog=EventMangementSystem;Integrated Security=True;TrustServerCertificate=True";
        ...
        public bool AddBooking(BookingTable booking)
        {
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                numberOfGuest, numberOfDays, functionTypes, image, serviceName, status, cateringItems)
                VALUES 
                (@uid, @fullName, @phoneNumber, @address, @date, 
                @numberOfGuest, @numberOfDays, @functionTypes, @image, @serviceName, @status, @cateringItems)";
                ...
                // 🔥 FIX (MAIN ISSUE)
                cmd.Parameters.Add("@serviceName", SqlDbType.NVarChar).Value =
                    string.IsNullOrEmpty(booking.serviceName) ? "" : booking.serviceName;

                cmd.Parameters.AddWithValue("@status", booking.status ?? "Pending");
                cmd.Parameters.AddWithValue("@cateringItems", booking.cateringItems ?? "");

                return cmd.ExecuteNonQuery() > 0;
            }
        }

        // ✅ GET ALL BOOKINGS (SAFE)
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                while (reader.Read())
                {
                    ...
                    list.Add(new BookingTable
                    {
                        ...
                        // 🔥 SAFE FIX
                        serviceName = reader["serviceName"] == DBNull.Value
                            ? ""
                            : reader["serviceName"].ToString(),

                        cateringItems = reader["cateringItems"].ToString(),
                    });
                }
            }
        ...
        // ✅ GET BOOKINGS BY USER ID (NEW)
        public List<BookingTable> GetBookingsByUserId(int uid)
        {
            List<BookingTable> list = new List<BookingTable>();
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * FROM Booking WHERE uid = @uid ORDER BY date DESC", conn);
