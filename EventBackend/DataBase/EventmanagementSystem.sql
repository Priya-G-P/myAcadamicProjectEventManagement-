...
CREATE TABLE Booking(
    ...
    cateringItems VARCHAR(MAX)   -- ✅ CATERING
);
...
CREATE TABLE Reviews (
    ...
    FOREIGN KEY (UserId) REFERENCES signup(uid)
);

-- Insert dummy review for testing
INSERT INTO Reviews (UserId, Rating, ReviewText, ReviewDate, Status) 
VALUES (101, 5, 'The catering was exquisite, and the entire event ran flawlessly! Cannot recommend them enough.', GETDATE(), 'Approved');

-- ==========================================
-- QUICK FIX FOR BOOKING TABLE
-- If you already have the Booking table created, just run this to add the Catering column:
-- ==========================================
-- ALTER TABLE Booking ADD cateringItems VARCHAR(MAX);

-- ==========================================
-- CATERING ITEMS SYSTEM
-- ==========================================

IF OBJECT_ID('CateringItems', 'U') IS NOT NULL 
    DROP TABLE CateringItems;

CREATE TABLE CateringItems (
    [Id] INT PRIMARY KEY IDENTITY(1,1),
    [Category] NVARCHAR(50) NOT NULL, -- e.g., 'Starter', 'MainCourse', 'Dessert'
    [ItemName] NVARCHAR(100) NOT NULL
);

-- Insert dummy data for initialization
INSERT INTO CateringItems (Category, ItemName) VALUES 
('Starter', 'Crispy Paneer Tikka'),
('Starter', 'Spring Rolls'),
('MainCourse', 'Butter Chicken'),
('MainCourse', 'Vegetable Biryani'),
('Dessert', 'Gulab Jamun'),
